...
public class Main1040EZForm extends JFrame {
    ...
    public static final int X_POS = 25;
    public static final int LABEL_WIDTH = 410;
    public static final int ENTRY_WIDTH = 100;
    ...
    UserNumberEntry interestIncome;
    UserNumberEntry unemploymentIncome;
    UserNumberEntry combatPay;
    UserNumberEntry witholding;
     ...
     */
    JTextField amountOfChildrenEntry;
    ...
    JTextArea taxSummary;
    String taxSummaryMessage = "";
    ...
    private void incomePage() {
        ...
        w2Income = new UserNumberEntry(
                "Wages, salaries, and tips. Enter total income from Box 1 of Form(s) W-2:",
                X_POS, LABEL_WIDTH, ENTRY_WIDTH, 15, HEIGHT, MARGIN
        );
        interestIncome = new UserNumberEntry(
                "Taxable Interest. If the total is over $1,500, you cannot use this Form:",
                X_POS, LABEL_WIDTH, ENTRY_WIDTH, 45, HEIGHT, MARGIN
        );
        unemploymentIncome = new UserNumberEntry(
                "Unemployment compensation and Alaska Permanent Fund dividends:",
                X_POS, LABEL_WIDTH, ENTRY_WIDTH, 75, HEIGHT, MARGIN
        );
        combatPay = new UserNumberEntry(
                "Nontaxable combat pay election:",
                X_POS, LABEL_WIDTH, ENTRY_WIDTH, 105, HEIGHT, MARGIN
        );
        witholding = new UserNumberEntry(
                "Income Tax Withheld:",
                X_POS, LABEL_WIDTH, ENTRY_WIDTH, 135, HEIGHT, MARGIN
        );

        w2Income.addToPanel(incomeEntryPanel);
        interestIncome.addToPanel(incomeEntryPanel);
        unemploymentIncome.addToPanel(incomeEntryPanel);
        combatPay.addToPanel(incomeEntryPanel);
        witholding.addToPanel(incomeEntryPanel);
        ...
        InvalidInputVerifier interestIncomeVerification = new InvalidInputVerifier(panel, interestIncome, 1500.0,
                "Your interest income is in excess of $1,500. You cannot use this form.");

        InvalidInputVerifier posNumberOrZero = new InvalidInputVerifier(panel);

        w2Income.setVerification(w2Verification);
        interestIncome.setVerification(interestIncomeVerification);
        unemploymentIncome.setVerification(posNumberOrZero);
        witholding.setVerification(posNumberOrZero);
        combatPay.setVerification(posNumberOrZero);
    }
...
//        String taxSummaryMessage = "Your taxes are estimated at $" + "X" + ". This is " + "X" + "% of your total income of " +
//                "$" + "X" + ". Your total tax payments for the year were $" + "X" + ". Your outstanding tax bill is estimated at $" + "X" +". " +
//                "This puts you in the " + "X" + "% tax bracket.\n";

        taxSummary = new JTextArea("");
        taxSummary.setBounds(560, 200, 300, 30*5);
        taxSummary.setEnabled(false);
        taxSummary.setLineWrap(true);
        taxSummary.setBackground(backgroundColor);
        taxSummary.setDisabledTextColor(Color.BLACK);
        finalTaxFormPanel.add(taxSummary);
...
//                w2incomeFormDisplay.setText(w2IncomeEntry.getText());
                interestIncomeFormDisplay.setText(interestIncome.getEntryText());
                unemploymentIncomeFormDisplay.setText(unemploymentIncome.getEntryText());
                combatPayFormDisplay.setText(combatPay.getEntryText());
                withholdingFormDisplay.setText(witholding.getEntryText());
                ...
                taxBracket = taxAmountCalculator.singleTaxBracket(Double.parseDouble(taxableIncomeFormDisplay.getText()));
                if(mfjBox.isSelected()) {
                    taxBracket = taxAmountCalculator.mfjTaxBracket(Double.parseDouble(taxableIncomeFormDisplay.getText()));
                }
                /*
                ...
                int combatIncome = Integer.parseInt(combatPay.getEntryText());
                ...
                paymentsAndCreditsFormDisplay.setText(taxComps.calcTotalPaymentsAndCredits(witholding.getEntryText(),
                ...
                DecimalFormat moneyDecimalFormat = new DecimalFormat("#,##0.00");
                DecimalFormat percentIncomeFormat = new DecimalFormat("##0.0");
                ...
                if (taxOrRefund > 0) {
                    ...
                    taxSummaryMessage += ". Your outstanding tax bill is estimated at $" +
                            moneyDecimalFormat.format(taxOrRefund);
                    ...
                    taxDueOrRefundFormDisplay.setText(taxOrRefundString);
                    taxSummaryMessage += ". Your outstanding tax refund is estimated at $" +
                            moneyDecimalFormat.format(taxOrRefund);
                }
                taxSummaryMessage += ". This puts you in the " + percentIncomeFormat.format(taxBracket*100.0) +
                        "% tax bracket.\n";

                taxSummary.insert(taxSummaryMessage, 0);
